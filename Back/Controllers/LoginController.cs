// LoginController.cs
// Endpoint de compatibilidad en /api/login (redirige lógica al AuthController).
// El endpoint principal es POST /auth/login.
...
public class LoginController : ControllerBase
{
    ...
    /// <summary>
    /// Login por ruta legada /api/login.
    /// Se recomienda usar POST /auth/login en su lugar.
    /// </summary>
    [AllowAnonymous]
    [HttpPost]
    public async Task<IActionResult> Login([FromBody] ApiLoginRequest request)
    {
        ...
        string campoId = request.Username.Contains('@') ? "email" : "username";

        var (codigo, mensaje) = await _servicioCrud.VerificarContrasenaAsync(
            "usuario", null, campoId, "password", request.Username, request.Password);
        ...
        var usuarios = await _servicioCrud.ObtenerPorClaveAsync("usuario", null, campoId, request.Username);
    ...
    private async Task<List<string>> ObtenerRolesUsuarioAsync(string usuarioId)
    {
        ...
        foreach (var relacion in relaciones)
        {
            ...
            if (rol.TryGetValue("activo", out var activoObj) && !EsActivo(activoObj)) continue;
            roles.Add(nombre);
        }
...
/// <summary>Request para el endpoint legado /api/login</summary>
